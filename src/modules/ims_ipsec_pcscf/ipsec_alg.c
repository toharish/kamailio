...
{
	if (!ealg || !ealg->s) return 0;
	if (ealg->len == 7 && strncasecmp(ealg->s, "aes-gcm", 7) == 0) return 1;
	if (ealg->len == 11 && strncasecmp(ealg->s, "aes-256-gcm", 11) == 0) return 1;
	return 0;
}

int is_auth_trunc_alg(const str *alg)
{
	if (!alg || !alg->s) return 0;
	if (alg->len == 16 && strncasecmp(alg->s, "hmac-sha-256-128", 16) == 0) return 1;
	return 0;
}

static int hex2byte(const str *hex, unsigned char *byte, int byte_len)
{
	if (!hex || !hex->s || !byte) return -1;
	if (hex->len < byte_len * 2) return -1;
	int i;
...
int ipsec_put_aead_attr(struct nlmsghdr *nlh, const str *ck, const str *ik, const str *ealg)
{
	if (!nlh || !ck || !ealg || !ck->s || !ealg->s) return -1;
	if (!is_aead_alg(ealg)) return -1;
	...
	if (ck->len >= total_key_bytes * 2) {
		if (hex2byte(ck, (unsigned char *)aead_algo->alg_key, total_key_bytes) < 0) {
		...
		if (ik && ik->s && ik->len >= 8) {
			if (hex2byte(ik, (unsigned char *)aead_algo->alg_key + key_bytes, 4) < 0) {
...
int ipsec_put_auth_trunc_attr(struct nlmsghdr *nlh, const str *ik, const str *alg)
{
	if (!nlh || !ik || !alg || !ik->s || !alg->s) return -1;
