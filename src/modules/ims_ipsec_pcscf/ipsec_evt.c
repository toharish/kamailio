...
{
	if (nlh->nlmsg_type != XFRM_MSG_EXPIRE || mnl_nlmsg_get_payload_len(nlh) < sizeof(struct xfrm_user_expire)) {
		return MNL_CB_OK;
	}
...
#ifndef _IPSEC_SPI_LIST_TEST
	if (ul.on_sa_expire) {
		ul.on_sa_expire(NULL, hard, (void *)(uintptr_t)spi);
	}
#endif
	return MNL_CB_OK;
}
...
void ipsec_start_expire_listener(void)
{
	...
	LM_INFO("XFRM Netlink Expire Listener initialized (listening on group XFRMGRP_EXPIRE)\n");
